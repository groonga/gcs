...
Domain.prototype = {
  ...
  getName: function(source) {
    ...
    if (source.headers && source.headers['Host']) {
      ...
      var domainNameFromHost = this.getDomainNameFromHost(host);
  ...
  getDomainNameFromHost: function(host) {
    var hostName = ;
    var domainMatcher = /^(?:doc|search)-(.+)-([^\.\-]+)\.us-east-1\./;
    var match = hostName.match(domainMatcher);
    if (match)
      return match[1];
