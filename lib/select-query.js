...
SelectQuery.prototype = {
  ...
  prepareSortKey: function() {
    var rank = this.rankExpression;
    var fieldName = rank.replace(/^-/, '');
    if (fieldName == 'text_relevance')
      return null;
  ...
  get rankExpression() {
    if (this._rankExpression === undefined)
      this._rankExpression = this.prepareRankExpression();
    return this._rankExpression;
  },
  prepareRankExpression: function() {
    var rank = this.request.query.rank;
    return rank || '-text_relevance';
  },
