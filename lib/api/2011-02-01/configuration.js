...
function createSenderErrorResponse(errorCode, error) {
  return createCommonErrorResponse('Sender', errorCode, error);
}
...
exports.createHandler = function(context, config) {
  ...
  return function(request, response, next) {
    ...
    try {
      ...
      logger.error(error);
      var errorCode = error.isValidationError ? 'ValidationError' : 'InternalFailure' ;
      var body = createSenderErrorResponse(errorCode, error);
      response.contentType('application/xml');
